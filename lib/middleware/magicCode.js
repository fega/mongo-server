...
const rand = (length, current) => {
  ...
  return length ? rand(--length, '0123456789'.charAt(Math.floor(Math.random() * 60)) + _current) : _current;
};
const getConfig = (obj = {}) => ({
  emailField: 'email',
...
exports.magicCodeVerifyToken = (config, db, magicResources) => async (req, res, next) => {
  ...
  return res.send({ state: 'success', $token });
};
