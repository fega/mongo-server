const { isString, isMatch, curry } = require('lodash');
...
const getDynamicPermissions = (permissions) => {
  ...
  if (isStr) return [[permissions]];
  if (permissions.every(isString)) return [permissions];
  /**
   * process
   */
...
const checkStaticPermissions = (permissions) => {
  ...
  return (req, res, next) => {
    ...
    if (p && !req.user) return next(new HttpError.Forbidden());

    const { permissions: userPermissions } = req.user;
    const pass = p.some(curry(isMatch)(userPermissions));
...
const checkDynamicPermissions = (permissions, specialPermissions = {}, filters = {}) => {
  ...
  return (req, res, next) => {
    ...
    const permissionParameter = {
      req, res, next, HttpError, user: req.user, resources: res.locals.resources,
    };
    /**
     * Dynamic Check
     */
...
const magicLinkCreateToken = (config, db) => {
  const {
    emailField, exp, redirectTo, template,
  } = config;
  return (req, res, next) => {
    /**
     * verify email
...
const magicLinkVerifyToken = (config, db) => {
  const {
    fn, emailField, exp, redirectTo, template,
  } = config;
  return (req, res, next) => {
    /**
     * Search token in db
...
const magicLinkGetUserJwt = (config, db) => {
  const {
    fn, emailField, exp, redirectTo, template,
  } = config;
  return (req, res, next) => {
