...
const {
  flatten, omit, get, mapValues, pick, omitBy, isString, reduce, merge,
} = require('lodash');
...
const getFilterValue = (value, key) => {
  ...
  if (modifier === 'nin') return { $nin: Array.isArray(value) ? coerceFilterValue(value, coerce) : [coerceFilterValue(value, coerce)] };
  if (modifier === 'in') return { $in: Array.isArray(value) ? coerceFilterValue(value, coerce) : [coerceFilterValue(value, coerce)] };
 ...
 */
const getLookupPipeline=(config, localField, foreignField)=>{
  const pipeline = [{
    $match:{
 ...
 */
const getPopulatePipeline = ($populate, resource, config) => {
  ...
  const lookupPipeline1 = config
    ? { 
      pipeline: getLookupPipeline(config, '_id', foreignField), 
      let: {[foreignField]: `$_id`} }
    : false
  const lookupPipeline2 = config
    ? { 
      pipeline: getLookupPipeline(config, '_id', foreignField+'s'), 
 ...
 */
exports.getPopulatePipelines = ($populate, $fill, resource, populateConfig={}) => {
  const result = [];
  if (Array.isArray($populate)) {

    $populate.forEach(populate => result
      .push(...getPopulatePipeline(populate, resource, populateConfig[populate])));
  } else if ($populate) {
    result.push(...getPopulatePipeline($populate, resource, populateConfig[$populate]));
  }
