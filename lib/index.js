...
const getFilterValue = (value, key) => {
  ...
  if (split[1] === 'ne') return { $ne: coerceFilterValue(value) };
  if (split[1] === 'gt') return { $gt: coerceFilterValue(value) };
  if (split[1] === 'lt') return { $lt: coerceFilterValue(value) };
  if (split[1] === 'lte') return { $lte: coerceFilterValue(value) };
  if (split[1] === 'gte') return { $gte: coerceFilterValue(value) };
  if (split[1] === 'size') return { $size: coerceFilterValue(value) };
  if (split[1] === 'date') return new Date(value);
